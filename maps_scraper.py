...
class MapsScraper:
    ...
    def _scrape_single(cls, context, shop, query):
        ...
        try:
            ...
            # Resilient Search box locator
            search_box = None
            chosen_selector = None
            
            # 1. page.get_by_role("textbox")
            try:
            ...
            if not search_box:
                ...
                for selector, sel_name in selectors_to_try:
                    try:
            ...
            if not search_box:
                ...
                err_screenshot_path = os.path.join(SCREENSHOTS_DIR, f"error_no_searchbox_{safe_name}_{shop.row_index}.png")
                page.screenshot(path=err_screenshot_path)
                
                # HTML Capture
                html_path = os.path.join(SCREENSHOTS_DIR, f"error_no_searchbox_{safe_name}_{shop.row_index}.html")
                with open(html_path, "w", encoding="utf-8") as hf:
                    hf.write(page.content())
                
                app_logger.error(f"Saved no-searchbox screenshot and HTML for {shop.shop_name}")
                app_logger.error(f"Current URL: {curr_url}")
                app_logger.error(f"Current Title: {curr_title}")
            ...
            # If multiple results appear, click the first valid business card
            ...
            if title_loc.count() > 0:
                shop.business_name = title_loc.first.inner_text().strip()
            else:
                shop.business_name = shop.shop_name
                
            # 2. Address
            address_locator = page.locator("button[data-item-id='address']")
            if address_locator.count() > 0:
                shop.latest_address = address_locator.inner_text().strip()
            ...
            if phone_locator.count() > 0:
                shop.phone_number = phone_locator.inner_text().strip()
                
            # 5. Website
            website_locator = page.locator("a[data-item-id='authority']")
            if website_locator.count() > 0:
                shop.website = website_locator.get_attribute("href")
                
            # 6. Business Category
            category_locator = page.locator("button[jsaction*='category']").first
            if category_locator.count() > 0:
                shop.business_category = category_locator.inner_text().strip()
            else:
                category_fallback = page.locator(".DkEaCc").first
                if category_fallback.count() > 0:
                    shop.business_category = category_fallback.inner_text().strip()
            ...
            if rating_container.count() > 0:
                container_text = rating_container.inner_text().strip()
                rating_match = re.search(r'^(\d\.\d)', container_text)
                if rating_match:
                    shop.rating = rating_match.group(1)
                reviews_match = re.search(r'\((\d+[,.\d]*)\)', container_text)
                if reviews_match:
                    shop.review_count = reviews_match.group(1).replace(",", "")
            else:
                rating_fallback = page.locator("span.fontBodyMedium").first
                if rating_fallback.count() > 0:
                    fb_text = rating_fallback.inner_text().strip()
                    rating_match = re.search(r'^(\d\.\d)', fb_text)
                    if rating_match:
                        shop.rating = rating_match.group(1)
            ...
            # 11. Plus Code
            plus_code_locator = page.locator("button[data-item-id='oloc']")
            if plus_code_locator.count() > 0:
                shop.plus_code = plus_code_locator.inner_text().strip().replace(" Plus Code", "")
                
            # 12. Business Status
            page_text = page.locator("body").inner_text()
            if "Permanently closed" in page_text:
                shop.business_status = "CLOSED_PERMANENTLY"
            elif "Temporarily closed" in page_text:
                shop.business_status = "CLOSED_TEMPORARILY"
            else:
                shop.business_status = "OPERATIONAL"
                
            # 13. Place ID / CID extraction from URL or canonical link
            place_id = ""
            try:
                canonical_link = page.locator("link[rel='canonical']").first
                if canonical_link.count() > 0:
                    canonical_url = canonical_link.get_attribute("href")
                    if "1s0x" in canonical_url:
                        match = re.search(r'1s(0x[0-9a-fA-F]+:0x[0-9a-fA-F]+)', canonical_url)
                        if match:
            ...
            if not place_id:
                try:
                    match = re.search(r'ftid:(0x[0-9a-fA-F]+:0x[0-9a-fA-F]+)', page.url)
                    if match:
                        place_id = match.group(1)
                except Exception:
                    pass
            ...
            app_logger.error(f"Playwright error inside worker during extraction: {ex}")
            if nav_end is None:
                nav_end = time.perf_counter()
            ext_end = time.perf_counter()
            try:
                safe_name = sanitize_filename(shop.shop_name)
                error_screenshot_path = os.path.join(SCREENSHOTS_DIR, f"error_{safe_name}_{shop.row_index}.png")
                page.screenshot(path=error_screenshot_path)
                
                # HTML Capture on Failure
                html_path = os.path.join(SCREENSHOTS_DIR, f"error_{safe_name}_{shop.row_index}.html")
                with open(html_path, "w", encoding="utf-8") as hf:
                    hf.write(page.content())
    ...
    def _worker(cls):
        ...
        try:
            ...
            if browser.is_connected():
                ...
                cls._is_running = False
                cls._ready_event.set() # Don't hang producer forever
                return
            ...
            while cls._is_running:
                ...
                try:
                    ...
                    for strat_name, query_val in strategies:
                        ...
                        success, nav_t, ext_t, err_type = cls._scrape_single(context, temp_shop, query_val)
                        ...
                        if success:
                            ...
                            app_logger.info(f"Playwright Strategy '{strat_name}' for Row {shop.row_index+1}: Conf={temp_shop.confidence_score}")
                    ...
                    app_logger.error(f"Playwright error inside worker for shop {shop.original_shop_name_address}:\n{err_msg}")
